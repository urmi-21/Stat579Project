...
The clinical data for BRCA studies is stored in a dataframe *brcaDF*. Next, we downloaded mutation data using TCGABiolinks. After getting the mutation data, we joined the clinical table with the mutation table to have clinical information for each mutation in the mutation table.
...
colnames(brcaDF)[which(colnames(brcaDF)=="portions.analytes.aliquots.submitter_id")]="Tumor_Sample_Barcode"
...
Next, we downloaded gene expression data for the BRCA studies. 

```{r,message=FALSE,cache=TRUE,warning=FALSE,results="hide"}
#download data from github
brca_nontumor <- read_delim("https:/raw.github.com/mskcc/RNAseqDB/blob/master/data/normalized/brca-rsem-fpkm-tcga.txt.gz", "\t", escape_double = FALSE, trim_ws = TRUE)
brca_tumor <- read_delim("https:/raw.github.com/mskcc/RNAseqDB/blob/master/data/normalized/brca-rsem-fpkm-tcga-t.txt.gz", "\t", escape_double = FALSE, trim_ws = TRUE)
...
We now have two expression datasets for tumor and normal or non-tumor tissues. These datasets contains the gene expression patterns over the tumor samples and normal samples. The data dimentions for tumor expression data is `r dim(brca_tumor)` and for normal expression data is `r dim(brca_nontumor)`
